...
function ComposeEmailPage() {
	...
	const [blockRemoteContent, setBlockRemoteContent] = useState<boolean>(
		createDefaultAppSettings().blockRemoteContent,
	);
				...
				Promise.all([
					...
					.then(([contacts, recentRecipients]) => {
						...
						const existing = new Set(
							[
								...recipientListsByField[activeRecipientField],
								...blockedRecipientsByField[activeRecipientField],
							]
								.map((entry) => normalizeRecipientAddress(entry))
								.filter((entry): entry is string => Boolean(entry)),
						);
						const nextRows: AutoCompleteRow[] = mergeRecipientSuggestions(
							contacts,
							recentRecipients,
							existing,
							12,
	...
	const selectedCloudAccount = useMemo(
		() =>
			typeof cloudAccountId === 'number'
				? (cloudAccounts.find((account) => account.id === cloudAccountId) ?? null)
				: null,
	...
	const openCloudAttachmentPicker = useCallback(async () => {
		...
		try {
			...
			const initialAccountId =
				typeof cloudAccountId === 'number' && rows.some((row) => row.id === cloudAccountId)
					? cloudAccountId
					: rows[0].id;
	...
	async function onCloudRefresh() {
		if (typeof cloudAccountId !== 'number') return;
		await loadCloudItems(cloudAccountId, normalizeRequestedCloudPath(cloudPath, selectedCloudProvider), {
			force: true,
		});
	}
					...
					<div className="ui-surface-card flex h-full w-full flex-col overflow-hidden">
						<div className="border-b ui-border-default px-5 py-4">
							<div
								className={
									showFromSelector
										? 'grid grid-cols-1 gap-2 md:grid-cols-[300px_1fr]'
...
function CloudAttachmentPickerModal({
	...
	return (
		...
		>
			...
			{status && <div className="notice-warning border-t px-4 py-2 text-xs">{status}</div>}
		</Modal>
	);
}
...
function RecipientMultiInput({
	...
	return (
		...
		>
			...
			/>
			{invalidMessage ? <div className="text-danger w-full pl-1 text-[11px]">{invalidMessage}</div> : null}
		</div>
	);
}

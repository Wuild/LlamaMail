	...
	return (
		...
		>
			...
			{(onEditFolder || onRefreshFolder) && (
				<div className="absolute right-1 top-1/2 inline-flex -translate-y-1/2 items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
					{onRefreshFolder && (
						<Button
							type="button"
			...
			{customDragActive && (
				<div className="folder-item-drop-indicator pointer-events-none absolute -top-0.5 left-2 right-2 h-0.5 rounded-full" />
			)}
		</div>
	);
}

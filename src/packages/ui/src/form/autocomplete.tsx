...
import {AutoCompleteRow, FormInputProps} from './shared';
				...
				<div className="autocomplete-menu">
					{rows.map((row, index) => (
						<Button
							key={row.id}
							type="button"
							className={cn('autocomplete-option block justify-start text-left', index === activeRowIndex && 'is-active')}
						...
						>
							...
							{row.description ? <div className="autocomplete-option-description">{row.description}</div> : null}
						</Button>
					))}
				</div>
			)}
